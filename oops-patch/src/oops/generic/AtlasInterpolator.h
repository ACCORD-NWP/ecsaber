...
class AtlasInterpolator : public LocalInterpolatorBase,
 ...
 public:
  ...
  /// Interpolate patch::Variables from source fields to target fields (no
  /// mask).
  void apply(const patch::Variables& variables, const atlas::FieldSet& sourceFieldSet,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate patch::Variables from source fields to target fields.
  void apply(const patch::Variables& variables, const atlas::FieldSet& sourceFieldSet,
             const std::vector<bool>& mask,
             std::vector<double>& targetFieldsVec) const;

  /// Adjoint of interpolation from source to target fields (no mask).
  void applyAD(const patch::Variables& variables, atlas::FieldSet& sourceFieldSet,
  ...
  /// Interpolate patch::Variables from State to targetFields (no mask).
  template <typename MODEL>
  void apply(const patch::Variables& variables, const State<MODEL>& state,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate patch::Variables from State to targetFields.
  template <typename MODEL>
  void apply(const patch::Variables& variables, const State<MODEL>& state,
             const std::vector<bool>& mask,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate patch::Variables from Increment to target fields (no mask).
  template <typename MODEL>
  void apply(const patch::Variables& variables, const Increment<MODEL>& increment,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate patch::Variables from Increment to target fields.
  template <typename MODEL>
  void apply(const patch::Variables& variables, const Increment<MODEL>& inc,
             const std::vector<bool>& mask,
             std::vector<double>& targetFieldVec) const;

  /// Adjoint of interpolation from Increment to target fields (no mask).
  template <typename MODEL>
  void applyAD(const patch::Variables& variables, Increment<MODEL>& inc,
               const std::vector<double>& targetFieldVec) const;
 ...
 protected:
  ...
  /// Copy source fields in patch::Variables to new FieldSet (overridable).
  virtual atlas::FieldSet copySourceFields(
      const patch::Variables& variables, const atlas::FieldSet& sourceFields) const;

  /// Create target FieldSet to match source FieldSet (overridable).
  virtual atlas::FieldSet createTargetFields(
      const patch::Variables& variables,
      const atlas::FunctionSpace& targetFunctionSpace,
      const atlas::FieldSet& sourceFields) const;

  /// Create patch::Variables object for interpolation call (overridable).
  virtual patch::Variables createInterpVariables(const patch::Variables& inputVariables)
      const;
 ...
 private:
  ...
  size_t getTotalElements(const patch::Variables& variables,
...
template <typename MODEL>
void AtlasInterpolator::apply(const patch::Variables& variables,
...
void AtlasInterpolator::applyAD(
    const patch::Variables& variables, Increment<MODEL>& increment,
    const std::vector<double>& targetFieldVec) const {
