...
class AtlasInterpolator : public LocalInterpolatorBase,
 ...
 public:
  ...
  /// Interpolate JediVariables from source fields to target fields (no
  /// mask).
  void apply(const JediVariables& variables, const atlas::FieldSet& sourceFieldSet,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate JediVariables from source fields to target fields.
  void apply(const JediVariables& variables, const atlas::FieldSet& sourceFieldSet,
             const std::vector<bool>& mask,
             std::vector<double>& targetFieldsVec) const;

  /// Adjoint of interpolation from source to target fields (no mask).
  void applyAD(const JediVariables& variables, atlas::FieldSet& sourceFieldSet,
  ...
  /// Interpolate JediVariables from State to targetFields (no mask).
  template <typename MODEL>
  void apply(const JediVariables& variables, const State<MODEL>& state,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate JediVariables from State to targetFields.
  template <typename MODEL>
  void apply(const JediVariables& variables, const State<MODEL>& state,
             const std::vector<bool>& mask,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate JediVariables from Increment to target fields (no mask).
  template <typename MODEL>
  void apply(const JediVariables& variables, const Increment<MODEL>& increment,
             std::vector<double>& targetFieldVec) const;

  /// Interpolate JediVariables from Increment to target fields.
  template <typename MODEL>
  void apply(const JediVariables& variables, const Increment<MODEL>& inc,
             const std::vector<bool>& mask,
             std::vector<double>& targetFieldVec) const;

  /// Adjoint of interpolation from Increment to target fields (no mask).
  template <typename MODEL>
  void applyAD(const JediVariables& variables, Increment<MODEL>& inc,
               const std::vector<double>& targetFieldVec) const;
 ...
 protected:
  ...
  /// Copy source fields in JediVariables to new FieldSet (overridable).
  virtual atlas::FieldSet copySourceFields(
      const JediVariables& variables, const atlas::FieldSet& sourceFields) const;

  /// Create target FieldSet to match source FieldSet (overridable).
  virtual atlas::FieldSet createTargetFields(
      const JediVariables& variables,
      const atlas::FunctionSpace& targetFunctionSpace,
      const atlas::FieldSet& sourceFields) const;

  /// Create JediVariables object for interpolation call (overridable).
  virtual JediVariables createInterpVariables(const JediVariables& inputVariables)
      const;
 ...
 private:
  ...
  size_t getTotalElements(const JediVariables& variables,
...
template <typename MODEL>
void AtlasInterpolator::apply(const JediVariables& variables,
...
void AtlasInterpolator::applyAD(
    const JediVariables& variables, Increment<MODEL>& increment,
    const std::vector<double>& targetFieldVec) const {
