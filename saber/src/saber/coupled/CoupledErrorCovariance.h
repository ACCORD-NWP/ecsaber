...
class CoupledErrorCovariance
 ...
 public:
  ...
  CoupledErrorCovariance(const Geometry_ &, const oops::JediVariables &,
 ...
 private:
  ...
  /// JediVariables for each component model
  const std::vector<oops::JediVariables> incVars_;
    ...
    incVars_(oops::splitVariables(incVars, geom.geometry().variables()))
{
  ...
  if (params.interp.value() && params.commonOuterBlocks.value()) {
    ...
    oops::JediVariables overlap = xb1.variables();
...
std::unique_ptr<SaberBlockChainBase>
  ...
  oops::JediVariables outerVars(vars);
