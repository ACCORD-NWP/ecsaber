...
oops::JediVariables SaberCentralBlockParameters::getActiveVars(
  const oops::JediVariables & defaultVars) const {
...
SaberCentralBlock::SaberCentralBlock(const oops::GeometryData & outerGeom,
  ...
  if (params.singleBlock.value()) {
    ...
    for (const auto & groupParams : *params.groups.value()) {
      ...
      const oops::JediVariables varNames = groupParams.variables.value();

      // Define group variables
      oops::JediVariables groupVars;
      ...
      groupInnerVars_.push_back(oops::JediVariables(
      ...
      const oops::JediVariables currentOuterVars = groupAuxOuterBlockChains_.back() ?
