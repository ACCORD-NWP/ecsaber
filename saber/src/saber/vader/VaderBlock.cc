    ...
    vader_(params.vader, outerBlockConf.getSubConfiguration("vader"))
{
  ...
  oops::JediVariables neededVars = outerVars_;
  atlas::FieldSet xb_outer = xb.fieldSet();
  oops::JediVariables ingredientVars = innerVars_;
  ...
  const oops::JediVariables varsProduced = vader_.initTLAD(ingredientVars);
