...
void writeOutputMIOToNetcdf(std::vector<std::vector<double>> qclIncRegrArr,
  ...
  const oops::JediVariables vars(std::vector<std::string>{"qcl_coef", "qcf_coef",
    ...
    blockparams_(params)
{
  ...
  const oops::JediVariables stateVariables = params.mandatoryStateVars();
  ...
  // eval_mio_fields_nl)
  const oops::JediVariables extraStateVariables({oops::Variable{"cleff"},
                                             oops::Variable{"cfeff"}});
