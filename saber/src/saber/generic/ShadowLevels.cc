...
void ShadowLevels::directCalibration(const oops::FieldSets &) {
  ...
  for (const auto & group : groups_) {
    ...
    // Shadow levels thickness
    double dh = 0.8*L;

    // Minimum number of shadow levels
    const size_t nz = static_cast<size_t>((vcSpan+3.0*L)/dh);
    ...
    // Shadow levels span
    const double hSpan = static_cast<double>(nz-1)*dh;

    // Extension zone
    const double de = 0.5*(hSpan-vcSpan);
    oops::Log::test() << "- normalized extension: " << de/L << std::endl;

    // Shadow levels bounds
    const double hMin = vcMin-de;
    const double hMax = vcMax+de;

    // Set actual levels thickness
    dh = hSpan/static_cast<double>(group.nz_-1);
    oops::Log::test() << "- normalized thickness: " << dh/L << std::endl;

    // Define shadow levels
    std::vector<double> shadowLevels;
    for (size_t k = 0; k < group.nz_; ++k) {
      shadowLevels.push_back(hMin+static_cast<double>(k)*dh);
    }
    ASSERT(oops::is_close_relative(shadowLevels[group.nz_-1], hMax, 1.0e-12));
