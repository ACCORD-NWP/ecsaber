...
template<typename MODEL> class DRPLanczosEVILMinimizer : public oops::DRMinimizer<MODEL> {
  typedef oops::BMatrix<MODEL>             Bmat_;
  typedef oops::CostFunction<MODEL>        CostFct_;
  typedef oops::ControlIncrement<MODEL>    CtrlInc_;
  typedef oops::HtRinvHMatrix<MODEL>       HtRinvH_;
...
template<typename MODEL>
double DRPLanczosEVILMinimizer<MODEL>::solve(CtrlInc_ & dx, CtrlInc_ & dxh, CtrlInc_ & rr,
  ...
  for (int jiter = 0; jiter < maxiter; ++jiter) {
    ...
    if (normReduction < tolerance) {
      oops::Log::info() << "DRPLanczos: Achieved required reduction in residual norm." << std::endl;
      break;
    }
  }
