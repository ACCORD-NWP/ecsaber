...
void applyRecipNtimesNplus1SpectralScaling(
    const oops::JediVariables& innerNames, const oops::JediVariables& outerNames,
...
GaussUVToGPWithSMV::GaussUVToGPWithSMV(
    const oops::GeometryData& outerGeometryData,
    const oops::JediVariables& outerVars, const eckit::Configuration& covarConf,
...
void GaussUVToGPWithSMV::multiply(oops::FieldSet3D& fset) const {
  ...
  applyRecipNtimesNplus1SpectralScaling(
      oops::JediVariables(std::vector<std::string>({"air_horizontal_divergence"})),
      oops::JediVariables(std::vector<std::string>({"air_horizontal_divergence"})),
