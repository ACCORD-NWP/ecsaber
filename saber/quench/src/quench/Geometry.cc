...
Geometry::Geometry(const eckit::Configuration & config,
  ...
  : comm_(comm), groups_(),
    config_(config), timeComm_(eckit::mpi::self()) {
  ...
  for (const auto & groupParams : params.groups.value()) {
    ...
    if (vert_coordParams != boost::none) {
      ...
      const Variables vert_coordVar(*vert_coordParamsFromFile);
    ...
    for (atlas::idx_t jlevel = 0; jlevel < group.levels_; ++jlevel) {
      double avg = 0.0;
      double counter = 0.0;
      for (atlas::idx_t jnode = 0; jnode < group.vert_coord_.shape(0); ++jnode) {
...
Geometry::Geometry(const Geometry & other)
  ...
  config_(other.config_), timeComm_(other.timeComm_) {
  oops::Log::trace() << classname() << "::Geometry starting" << std::endl;
...
std::vector<size_t> Geometry::variableSizes(const oops::JediVariables & vars) const {
