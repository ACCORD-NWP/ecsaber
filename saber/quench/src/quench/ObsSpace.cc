...
void ObsSpace::generateDistribution(const eckit::Configuration & config) {
  ...
  if (comm_.rank() == 0) {
    ...
    for (size_t jo = 0; jo < nobsGlb_; ++jo) {
      size_t offset = 0;
      for (size_t jt = 0; jt < partition[jo]; ++jt) {
...
void ObsSpace::read(const std::string & fileName) {
  ...
  if (comm_.rank() == 0) {
    // NetCDF IDs
    int retval, ncid, nobs_id, Locations_id, metaData_id, dateTime_id, longitude_id, latitude_id,
      height_id, col_id;
...
void ObsSpace::write(const std::string & fileName,
  ...
  if (comm_.rank() == 0) {
    ...
    if ((retval = nc_put_att_long(metaData_id, dateTime_id, fillValue_key.c_str(), NC_INT64, 1,
      &util::missingValue(0L)))) ERR(retval);
