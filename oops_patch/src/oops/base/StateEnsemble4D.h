...
template<typename MODEL> class StateEnsemble4D {
 ...
 public:
  ...
  explicit StateEnsemble4D(std::vector<State4D_> & stateSetVec, const int ensNum);
  ...
  State4D_ & operator[](const size_t ii) { return states_[ii]; }
  const State4D_ & operator[](const size_t ii) const { return states_[ii]; }
...
StateEnsemble4D<MODEL>::StateEnsemble4D(std::vector<State4D_> & stateSetVec, const int ensNum) :
  states_(stateSetVec) {
...
StateEnsemble4D<MODEL>::StateEnsemble4D(const Geometry_ & resol,
      const eckit::Configuration & config, State4D_ & stateSet) : states_() {
