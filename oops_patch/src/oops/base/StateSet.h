...
class StateSet : public DataSetBase< State<MODEL>, Geometry<MODEL> > {
 ...
 public:
  StateSet(const Geometry_ &, const JediVariables &,
...
// get vector of local configurations
  ...
  for (size_t jm = 0; jm < this->local_ens_size(); ++jm) {
    for (size_t jt = 0; jt < this->local_time_size(); ++jt) {
      const Model_ model(resol, locals.at(indx).getSubConfiguration("model"));
      this->dataset().emplace_back(new State_(resol, model, locals.at(indx)));
      ++indx;
    }
  }
