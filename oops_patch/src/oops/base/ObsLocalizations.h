...
template <typename MODEL>
ObsLocalizations<MODEL>::ObsLocalizations(const eckit::Configuration & config,
                                          const ObsSpaces_ & obspaces) {
  Log::trace() << "ObsLocalizations<MODEL>::create starting" << std::endl;
  ...
  for (size_t jj = 0; jj < obsconf.size(); ++jj) {
    ...
    for (size_t oli = 0; oli < obsLocConfigs.size(); ++oli) {
      tmpVector.emplace_back(ObsLocalizationFactory<MODEL>::
  ...
  Log::trace() << "ObsLocalizations<MODEL>::create done" << std::endl;
}
...
template<typename MODEL>
void ObsLocalizations<MODEL>::print(std::ostream & os) const {
  for (size_t jj = 0; jj < local_.size(); ++jj) {
    for (size_t oli = 0; oli < local_[jj].size(); ++oli) {
      if (local_[jj][oli]) {
