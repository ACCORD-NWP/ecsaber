...
class LocalEnsembleDAParameters : public Parameters {
  OOPS_CONCRETE_PARAMETERS(LocalEnsembleDAParameters, Parameters)
 ...
 public:
  ...
  OptionalParameter<JediVariables> incvars{"increment variables",
...
/*
  ...
  // LocalEnsembleSolver functionality.
    ...
    const JediVariables vars(ic, "state variables");
...
// -----------------------------------------------------------------------------
 ...
 private:
  std::string appname() const override {
    return "oops::LocalEnsembleDA<" + MODEL::name() + ">";
  }
  ...
  void calculate_patchCenter(const Geometry_ & geometry, std::vector<double> & patchCenter) const {
    ...
    double zmean = 0.0;
    int n = 0;
    for (auto i = geometry.geometry().begin(); i != geometry.geometry().end(); ++i) {
      gptmp3 = *i;
