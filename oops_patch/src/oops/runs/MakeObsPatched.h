...
class MakeObsPatched : public Application {
  using Departures_ = Departures<MODEL>;
  using Geometry_ = Geometry<MODEL>;
  using Model_ = Model<MODEL>;
  using ModelAux_ = ModelAuxControl<MODEL>;
  using ObsAuxCtrls_ = ObsAuxControls<MODEL>;
  using Observations_ = Observations<MODEL>;
  using ObsSpaces_ = ObservationSpaces<MODEL>;
  using ObsOperators_ = ObsOperators<MODEL>;
  using State_ = State<MODEL>;

 public:
  // -----------------------------------------------------------------------------
  MakeObsPatched() { instantiateObsErrorFactory<MODEL>(); }
  // -----------------------------------------------------------------------------
  virtual ~MakeObsPatched() {}
  // -----------------------------------------------------------------------------
  int execute(const eckit::Configuration& fullConfig) const {
    ...
    std::shared_ptr<Observer<MODEL, State_> > pobs(
        new Observer<MODEL, State_>(obspace, hop, ybias));
  ...
  // -----------------------------------------------------------------------------
 private:
  std::string appname() const { return "oops::MakeObsPatched<" + MODEL::name() + ">"; }
  // -----------------------------------------------------------------------------
};
