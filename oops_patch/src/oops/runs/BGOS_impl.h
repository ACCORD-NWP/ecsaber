...
void BGOS_impl(const eckit::Configuration& fullConfig) {
  ...
  Log::trace() << "BGOS: cost function has been set up" << std::endl;
  ...
  if (fullConfig.has("ensemble")) {
    ...
    util::expandEnsembleTemplate(membersConfigTemplate, nens);
  ...
  // Print global norm
  Log::test() << "Variance in observation space norm: " << 
