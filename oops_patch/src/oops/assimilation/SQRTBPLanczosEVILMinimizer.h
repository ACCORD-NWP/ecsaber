...
class SQRTBPLanczosEVILMinimizer : public SQRTMinimizer<MODEL> {
...
double SQRTBPLanczosEVILMinimizer<MODEL>::solve(
  ...
  while (jiter < maxiter) {
    ...
    if (linfoconv) {
      ...
      Log::info() << "SQRTBPLanczos end of iteration " << jiter + 1 << std::endl
  ...
  if (nvec > 0) {
    ...
    if (eigvals_.size() != 0 && eigvals_old.size() != 0) {
      if (eigvals_[0] > 1.01 * eigvals_old[0]) {
        // RITZ VALUES EXPLODE!
        ritzFailureCheckFlag = true;
        Log::info() << "SQRTBPLanczos: Ritz values explode" << std::endl
