...
class PLanczosEVILMinimizer : public PrimalMinimizer<MODEL> {
 ...
 public:
  ...
  PLanczosEVILMinimizer(const eckit::Configuration &conf,
                        const CostFct_ &J)
    : PrimalMinimizer<MODEL>(J), conf_(conf) {}
...
double PLanczosEVILMinimizer<MODEL>::solve(CtrlInc_ &dx,
  ...
  for (jiter = 0; jiter < ninner; ++jiter) {
    Log::info() << " PLanczosEVILMinimizer Starting Iteration " << jiter + 1 << std::endl;
    ...
    if (jiter == 0) {
      ...
      TriDiagSolve(ritzPairs.alphas(), ritzPairs.betas(), dd, yy);
    }
    ...
    Log::info() << "PLanczosEVILMinimizer end of iteration " << jiter + 1
                << ". Norm reduction= " << util::full_precision(reduc)
                << std::endl
                << std::endl;

    if (reduc < gnreduc) {
      Log::info() << "PLanczosEVILMinimizer: Achieved required reduction in residual norm."
                  << std::endl;
      break;
    }
  }
