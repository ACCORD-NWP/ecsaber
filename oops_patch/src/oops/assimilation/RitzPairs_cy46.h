...
void RitzPairs<VECTOR>::process(const eckit::Configuration & conf,
  ...
  Log::trace() << "RitzPairs::process start" << std::endl;
  ...
  // Compute eigenpairs
  Log::info() << "Compute eigenpairs" << std::endl;
  std::vector<double> evals;
  std::vector<std::vector<double>> evecs;
  TriDiagSpectrum(alphas_, betas_, evals, evecs);

  // Compute valid Ritz pairs
  Log::info() << "Compute valid Ritz pairs" << std::endl;
  ...
  for (int iiter1 = jiter - 1; iiter1 >= 0; --iiter1) {
    ...
    if (ebnd < ritzTol) {
      Log::test() << "Accept Ritz pair #" << (jiter - iiter1) << std::endl;
      Log::info() << "Eigen value: " << evals[iiter1] << " (" << ebnd << " < "
                  << ritzTol << ")" << std::endl;
      ...
      Log::test() << "Dismiss Ritz pair #" << (jiter - iiter1) << std::endl;
      Log::info() << "Eigen value: " << evals[iiter1] << " (" << ebnd << " < "
                  << ritzTol << ")" << std::endl;
    }
  }
  ...
  Log::trace() << "RitzPairs::process done" << std::endl;
}
