...
class GETKFSolver : public LocalEnsembleSolver<MODEL> {
...
//  typedef ObsDataVector<MODEL>        ObsDataInt_;
 ...
 public:
  ...
  Observations_ computeHofX(const StateEnsemble4D_ &, size_t, bool, const Model_ &,
                            const Observations_ &, const ObsAux_ &);

  /// entire KF update (computeWeights+applyWeights) for a grid point GeometryIterator__
  void measurementUpdate(const IncrementEnsemble4D_ &, const GeometryIterator__ &,
...
template <typename MODEL>
GETKFSolver<MODEL>::GETKFSolver(ObsSpaces_ & obspaces, const Geometry_ & geometry,
...
// -----------------------------------------------------------------------------
template <typename MODEL>
Observations<MODEL> GETKFSolver<MODEL>::computeHofX(const StateEnsemble4D_ & ens_xx,
                                                    size_t iteration,
                                                    bool readFromFile, const Model_ & model,
  ...
  ObsAux_  obsaux(this->obspaces_, this->obsconf_);
  ObsAuxInc_  obsauxinc(this->obspaces_, this->obsconf_);
  Observations_ yb_mean(this->obspaces_);

  if (readFromFile) {
    // compute/read H(x) for the original ensemble members
    // also computes omb_
    yb_mean = LocalEnsembleSolver<MODEL>::computeHofX(ens_xx, iteration, readFromFile, model,
                                                      yobs, ybias);
    ...
    for (size_t iens = 0; iens < nens_; ++iens) {
      ...
      for (size_t ieig = 0; ieig < neig_; ++ieig) {
        ytmp.read(util::setObsValue(this->obsconf_, "hofxm"+std::to_string(iteration)+"_"+
                      std::to_string(ieig+1)+"_"+std::to_string(iens+1)));
        for (size_t jj = 0; jj < HZb_[ii].size(); ++jj) {
          HZb_[ii][jj] = ytmp[jj];
          HZb_[ii][jj] -= yb_mean[jj];
        }
        ii = ii + 1;
      }
    }
...
/*
      ...
      for (size_t iens = 0; iens < ens_xx.size(); ++iens) {
        util::mask(*this->invVarR_, this->Yb_[iens]);
        util::mask(this->Yb_[iens], (this->invVarR_));
      }

      // calculate obs departures and mask with qc flag
      Observations_ yobs(this->obspaces_, "ObsValue");
      this->omb_ = yobs - yb_mean;
      util::mask(*this->invVarR_, this->omb_);
      util::mask(this->omb_, *this->invVarR_);
...
*/
      ...
      for (size_t iens = 0; iens < nens_; ++iens) {
        ...
        for (size_t ieig = 0; ieig < neig_; ++ieig) {
          ...
          for (size_t jj = 0; jj < tmpState.statesNumber(); ++jj) {
            tmpState[jj] += Ztmp[ieig][jj];
          }
          Observations_ tmpObs(this->obspaces_);
          this->computeHofX4DNonLinear(config, model, tmpState, ybias, tmpObs);
          for (size_t jj = 0; jj < HZb_[ii].size(); ++jj) {
            HZb_[ii][jj] = tmpObs[jj];
            HZb_[ii][jj] -= yb_mean[jj];
          }
          tmpObs.save(util::setObsValue(this->obsconf_, "hofxm"+std::to_string(iteration)+"_"
                        +std::to_string(ieig+1)+"_"+std::to_string(iens+1)));
          ii = ii + 1;
        }
      }
...
template <typename MODEL>
void GETKFSolver<MODEL>::computeWeights(const Eigen::VectorXd & dy,
...
template <typename MODEL>
void GETKFSolver<MODEL>::applyWeights(const IncrementEnsemble4D_ & bkg_pert,
  ...
  for (unsigned itime=bkg_pert[0].first(); itime <= bkg_pert[0].last(); ++itime) {
...
template <typename MODEL>
void GETKFSolver<MODEL>::measurementUpdate(const IncrementEnsemble4D_ & bkg_pert,
                                           const GeometryIterator__ & i,
                                           IncrementEnsemble4D_ & ana_pert) {
  ...
  for (size_t iens = 0; iens < nanal_; ++iens) {
    util::mask(*this->invVarR_, this->HZb_[iens]);
  }
  ...
  if (local_omb_vec.size() == 0) {
    ...
    const Eigen::VectorXd localization = util::packEigen(locvector, locvector);
