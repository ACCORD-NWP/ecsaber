...
double RPLanczosEVILMinimizer<MODEL>::solve(Dual_ &vv, double &vvp, Dual_ &rr,
  ...
  for (jiter = 0; jiter < maxiter; ++jiter) {
    ...
    if (jiter == 0) {
      ...
      TriDiagSolve(ritzPairs.alphas(), ritzPairs.betas(), d, y);
    }
    ...
    Log::info() << "RPLanczos end of iteration " << jiter + 1
                << ". Norm reduction= " << util::full_precision(normReduction)
                << std::endl
                << std::endl;

    if (normReduction < tolerance) {
      Log::info() << "RPLanczos: Achieved required reduction in residual norm."
                  << std::endl;
      break;
    }
  }
